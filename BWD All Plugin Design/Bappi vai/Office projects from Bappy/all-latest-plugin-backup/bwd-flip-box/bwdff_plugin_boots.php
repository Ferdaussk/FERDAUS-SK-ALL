...
class ClassBWDFlipBox {
	...
	function bwdff_add_elementor_widget_categories( $elements_manager ) {
		...
		$elements_manager->add_category(
			...
			[
				'title' => esc_html__( 'BWD Flip Box', 'bwd-flip-flop' ),
				'icon' => 'eicon-person',
			]
		);
	}
	public function bwdff_all_assets_for_the_public(){
		...
		// font awesome
		wp_enqueue_style( 'bwdff_fs', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.9.0/css/all.min.css', array(),'5.9.0','all');

		// bootstrap css
		wp_enqueue_style( 'bwdff_bs', 'https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css', array(),'5.9.0','all');
        ...
        foreach($all_css_js_file as $handle => $fileinfo){
            wp_enqueue_style( $handle, $fileinfo['bwdff_path_define'], null, '1.1', 'all');
            wp_enqueue_script( $handle, $fileinfo['bwdff_path_define'], ['jquery'], '1.1', true);
        }
	}
	public function bwdff_all_assets_for_elementor_editor_admin(){
        ...
        foreach($all_css_js_file as $handle => $fileinfo){
            wp_enqueue_style( $handle, $fileinfo['bwdff_path_admin_define'], null, '1.1', 'all');
        }
	}
