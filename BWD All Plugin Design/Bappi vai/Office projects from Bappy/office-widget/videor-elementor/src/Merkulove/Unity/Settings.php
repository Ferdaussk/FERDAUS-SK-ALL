...
final class Settings {
        ...
        <aside class="mdc-drawer">
            <div class="mdc-drawer__content">
                <nav class="mdc-list">
                    ...
                    <h6 class="mdc-list-group__subheader"><?php echo esc_html__( 'Plugin settings', 'videor-elementor' ) ?></h6>
					<?php
					...
					foreach ( $tabs as $tab => $value ) {
						...
						/** Prepare link. */
						$link = admin_url( 'admin.php?page=mdp_videor_elementor_settings&tab=' . $tab );
	...
	private function support_link() {
	    ...
	    ?>
        <hr class="mdc-list-divider">
        <h6 class="mdc-list-group__subheader"><?php echo esc_html__( 'Helpful links', 'videor-elementor' ) ?></h6>

        <a class="mdc-list-item" href="https://docs.merkulov.design/tag/videor" target="_blank">
            <i class="material-icons mdc-list-item__graphic" aria-hidden="true">collections_bookmark</i>
            <span class="mdc-list-item__text"><?php echo esc_html__( 'Documentation', 'videor-elementor' ) ?></span>
        </a>

		<?php if ( TabActivation::get_instance()->is_activated() ) : /** Activated. */ ?>
            <a class="mdc-list-item" href="https://1.envato.market/videor-elementor-support" target="_blank">
                <i class="material-icons mdc-list-item__graphic" aria-hidden="true">mail</i>
                <span class="mdc-list-item__text"><?php echo esc_html__( 'Get help', 'videor-elementor' ) ?></span>
            </a>
            <a class="mdc-list-item" href="https://1.envato.market/cc-downloads" target="_blank">
                ...
                <span class="mdc-list-item__text"><?php echo esc_html__( 'Rate this plugin', 'videor-elementor' ) ?></span>
            </a>
		<?php endif; ?>
        ...
        <a class="mdc-list-item" href="https://1.envato.market/cc-merkulove" target="_blank">
            ...
            <span class="mdc-list-item__text"><?php echo esc_html__( 'More plugins', 'videor-elementor' ) ?></span>
        </a>
		<?php
	...
	private function add_submenu_elementor() {
        ...
        add_submenu_page(
            $parent,
            esc_html__( 'Videor Settings', 'videor-elementor' ),
            esc_html__( 'Videor for Elementor', 'videor-elementor' ),
            'manage_options',
            'mdp_videor_elementor_settings',
            [ $this, 'options_page' ]
        );
    ...
    private function render_nag_saved() {
        ...
        /** Render "Settings Saved" message. */
        UI::get_instance()->render_snackbar( esc_html__( 'Settings saved!', 'videor-elementor' ) );
    ...
    private function render_nag_activation() {
        ...
        if ( TabActivation::get_instance()->is_activated() ) {
            ...
            UI::get_instance()->render_snackbar( esc_html__( 'Plugin activated successfully.', 'videor-elementor' ), 'success', 5500 );
            return;

        }

        /** Render "Activation failed" message. */
        UI::get_instance()->render_snackbar( esc_html__( 'Invalid purchase code.', 'videor-elementor' ), 'error', 5500 );
        ...
        <div class="mdc-drawer__header mdc-plugin-fixed">
            <a class="mdc-list-item mdp-plugin-title" href="#">
                <i class="mdc-list-item__graphic" aria-hidden="true">
                    <img src="<?php echo esc_attr( Plugin::get_url() . 'images/logo-color.svg' ); ?>" alt="<?php esc_html_e( 'VideorElementor', 'videor-elementor' ) ?>">
                </i>
                <span class="mdc-list-item__text">
                    <?php if ( 'wordpress' === Plugin::get_type() ) : ?>
                        <?php esc_html_e( 'Videor for Elementor', 'videor-elementor' ) ?>
                    <?php else: ?>
                        <?php esc_html_e( 'Videor', 'videor-elementor' ) ?>
                    <?php endif; ?>
                </span>
                <span class="mdc-list-item__text">
                    <sup>
                        <?php esc_html_e( 'v.', 'videor-elementor' ); ?>
            ...
            <button type="submit" name="submit" id="submit" class="mdc-button mdc-button--dense mdc-button--raised">
                <span class="mdc-button__label"><?php esc_html_e( 'Save changes', 'videor-elementor' ) ?></span>
            </button>
        </div>
		<?php
	...
	public function get_options() {
        ...
        foreach ( $tabs as $tab_slug => $tab ) {
	        ...
	        $opt_name = "mdp_videor_elementor_{$tab_slug}_settings";
